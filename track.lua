...
-- ================== SETTINGS ==================
local HOUSE_DELAYS =
    { [1] = 0.1, [2] = 0.1, [3] = 0.1, [4] = 15, [5] = 90, [6] = 200 }
...
local function SellAll()
    ...
    if #all > 0 then
        ...
        print("[AutoSell] Sold:", #all)
    end
end

-- LOOP KHÔNG BLOCK SCRIPT KHÁC
local function StartLoop()
    task.spawn(function()
        while true do
            if getgenv().Config.AutoSell then
                task.spawn(SellAll) -- chạy chức năng sell riêng biệt
            end

            -- delay KHÔNG block
            task.wait((getgenv().Config.DelayMinutes or 1) * 60)
        end
    end)
end
